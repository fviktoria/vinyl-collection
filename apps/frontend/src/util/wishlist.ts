...
export const getWishlistReserved = async (ids: string[]) => {
  ...
  const values: (UpstashWishlistItemInterface | null)[] =
    await redisClient.mget(ids);

  // match values with ids (=keys)
  const matchedValues: Record<string, UpstashWishlistItemInterface> =
    values.reduce(
      (acc, value, index) => {
...
export const undoSetItemAsReserved = async ({
  id,
  email,
}: SetReservedParams) => {
  const redisClient = getRedisClient();
